...
public:
    ...
    ~Subtraction();
    int value() const { return _minuend->value() - _subtrahend->value(); }
    void setMinuend(const Expression* m) { _minuend = m; }
    void setSubtrahend(const Expression* s) { _subtrahend = s; };
